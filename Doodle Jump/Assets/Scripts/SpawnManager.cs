...
public class SpawnManager : MonoBehaviour
{
    ...
    public float SpawnPlatforms(float firstY,int level)
    {
        ...
        SpawnCoin(firstY,numberOfPlatforms,5);
    ...
    private void SpawnCoin(float firstY, int numberOfPlats, int numberOfEnemies)
    {
        if (numberOfEnemies == 0)
            return;
        var mX = maxY * numberOfPlatforms / numberOfEnemies;
        ...
        for (int i = 1; i < numberOfEnemies; i++)
        {
            spawnPosition.y += Random.Range(3, mX);
            spawnPosition.x = Random.Range(-2.0f, 2.1f);
            var spawnedEnemy = Instantiate(_coinPrefab, spawnPosition, Quaternion.identity);
            spawnedEnemy.transform.parent = _coinContainer.transform;
        }
    }
}